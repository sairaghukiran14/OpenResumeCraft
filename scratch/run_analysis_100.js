...
const OUTPUT_PATH = '/Users/sairaghukiranavula/.gemini/antigravity-ide/brain/f6365133-5156-4229-8a52-c7e6a20d5eb6/analysis_results_100.md';
...
// Programmatic 100 JDs Generator focusing on Frontend/React Developer with 2 YOE
function generate100JobDescriptions() {
  const companies = [
...
async function main() {
  console.log('--- STARTING 100-JD BENCHMARK ANALYSIS ---');
  const startTime = Date.now();
  const startMemory = process.memoryUsage().heapUsed;

  const jds = generate100JobDescriptions();
  console.log(`Generated 100 Frontend/React Developer (2 YOE) job descriptions.`);
  ...
  const results = await promisePool(jds, CONCURRENCY, async (jd) => {
    ...
    console.log(`[JD #${jd.id}] ${jd.company} (${jd.platform}) - Before: ${beforeAnalysis.score}%, After: ${afterAnalysis.score}%, Cache: ${cached ? 'HIT' : 'MISS'}, Time: ${durationMs}ms`);
  ...
  let report = `# ATS Resume Customization Benchmark Report (100 Job Postings)
...
| Metric | Value |
| --- | --- |
| Average Baseline (Before) Score | ${(results.reduce((a, b) => a + b.beforeScore, 0) / 100).toFixed(2)}% |
| Average Optimized (After) Score | ${(results.reduce((a, b) => a + b.afterScore, 0) / 100).toFixed(2)}% |
| Average Score Improvement | +${((results.reduce((a, b) => a + b.afterScore, 0) - results.reduce((a, b) => a + b.beforeScore, 0)) / 100).toFixed(2)}% |
| Highest Baseline Score | ${Math.max(...results.map(r => r.beforeScore))}% |
| Highest Tailored Score | ${Math.max(...results.map(r => r.afterScore))}% |
| Lowest Tailored Score | ${Math.min(...results.map(r => r.afterScore))}% |

## 3. Score Distribution
| Range | Baseline count | Tailored count |
| --- | --- | --- |
| Excellent (>= 85%) | ${results.filter(r => r.beforeScore >= 85).length} | ${results.filter(r => r.afterScore >= 85).length} |
| Good (70% - 84%) | ${results.filter(r => r.beforeScore >= 70 && r.beforeScore < 85).length} | ${results.filter(r => r.afterScore >= 70 && r.afterScore < 85).length} |
| Low Alignment (< 70%) | ${results.filter(r => r.beforeScore < 70).length} | ${results.filter(r => r.afterScore < 70).length} |
...
## 5. Detailed Runs Table (First 15 representative runs)
| JD ID | Job Title & Company | Baseline Score | Tailored Score | Change | Cache Status | Time |
| --- | --- | --- | --- | --- | --- | --- |
${results.slice(0, 15).map(r => `| #${r.id} | ${r.title} @ ${r.company} | ${r.beforeScore}% | ${r.afterScore}% | +${r.afterScore - r.beforeScore}% | ${r.cached ? 'HIT' : 'MISS'} | ${r.durationMs}ms |`).join('\n')}
`;
